...
& node.exe (Join-Path $Root "scripts\install-codex-config.mjs") (Join-Path $Root "config\codex-cli-config.toml") (Join-Path $CodexHome "config.toml") $Root $HOME
...
if (-not (Test-Path (Join-Path $AccountHome "notion_account.json")) -and
    @(Get-ChildItem -LiteralPath (Join-Path $AccountHome "accounts") -Filter "*.json" -File -ErrorAction SilentlyContinue).Count -eq 0) {
    ...
    Write-Host "Then run: .\stop.ps1; .\start.ps1"
}
