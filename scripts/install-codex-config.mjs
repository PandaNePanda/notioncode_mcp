...
const [templatePath, destination, projectRoot, userHome] = process.argv.slice(2);
if (!templatePath || !destination || !projectRoot || !userHome) {
  throw new Error(
    "usage: install-codex-config.mjs <template> <destination> <project-root> <user-home>",
  );
}
