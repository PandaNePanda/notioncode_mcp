...
> cookie `token_v2`, равную по чувствительности паролю. Не отправляйте её в чат,
> issue, лог или командную строку. Проверьте правила Notion и используйте проект
> на свой риск. Порты bridge по умолчанию доступны только на `127.0.0.1`.
...
Проверьте credential и перезапустите bridge:

```bash
sudo -u "$USER" -H "$PWD/.runtime/notion-agent-cli-venv/bin/notion-agent" \
  doctor --account "$HOME/.notionagents/notion_account.json" --json
sudo systemctl restart notion-fable-proxy.service
```
...
- Не проси пользователя отправить `token_v2` в чат.
- Не вставляй `token_v2` в аргумент команды, `.env`, README или лог.
...
5. Запусти ровно один штатный installer для ОС. Не собирай свою конфигурацию
   вручную.
6. Проверь наличие `~/.notionagents/notion_account.json`.
7. Если credential отсутствует, остановись у команды `notion-agent init
   ...
   Не читай токен и не повторяй его в ответе.
8. Выполни `notion-agent doctor`, затем перезапусти bridge.
9. Выполни health/verify и проверь число configured accounts.
